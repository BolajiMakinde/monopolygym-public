...
class FixedPolicyFourPlayer(Player):
    def __init__(self, name: str, mgn_code: str, starting_balance: int = 1500):
        ...
        self.policy_name = "FixedPolicyFour (Utility Tycoon)"
        self.safety_buffer_general = 100
        self.min_cash_after_buy_via_mortgage = 0 # Willing to go to 0 for a utility
    ...
    def decide_actions(self, game_state: State) -> List[Action]:
        ...
        if game_state.auction_state and game_state.auction_state.player_to_act == player:
            ...
            bid_limit = 0
            if isinstance(prop_on_auction, Utility):
                bid_limit = prop_on_auction.purchase_cost * 1.5 # Bid high for utilities
            elif isinstance(prop_on_auction, Railroad):
                bid_limit = prop_on_auction.purchase_cost * 1.1
            else: # Street or other
        ...
        # --- 0. Trade Response (Always Decline) ---
        ...
        # --- 1. Jail Actions ---
        if player.in_jail:
            if player.balance >= 50 + self.safety_buffer_general and is_action_type_valid('PayJailFineAction'): # Pay first
        ...
        if player.balance < 0:
            ...
            if is_action_type_valid('MortgageAction'):
                ...
                props_to_consider_mortgaging = sorted(
                    unmortgaged_props,
                    key=lambda p_obj: (
                        isinstance(p_obj, Utility),  # Mortgage utilities last
                        isinstance(p_obj, Railroad), # Mortgage railroads second to last
        ...
        # --- 3. Pre-roll Strategic: Unmortgage Utilities/Railroads if cash rich ---
        if is_action_type_valid('UnmortgageAction'):
            if player.balance > 750: 
                mortgaged_rrs_utils = [p for p in player.properties if p.is_mortgaged and (isinstance(p, Utility) or isinstance(p, Railroad))]
        ...
        # --- 4. Pre-roll Strategic: Build Houses (Rarely) ---
        num_owned_rrs = self._count_owned_railroads(game_state)
        num_owned_utils = self._count_owned_utilities(game_state)
        if is_action_type_valid('BuildAction') and num_owned_utils >= 2 and num_owned_rrs >=4 and player.balance > 1000:
            # Logic similar to FixedPolicyThree for building, just different trigger condition
            ...
            for prop in player.properties:
                if isinstance(prop, Street) and prop.color_set and self._owns_full_color_set(game_state, prop):
                    ...
                    for s_in_set in self._get_all_streets_in_color_set(game_state, prop.color_set):
                        if s_in_set.is_mortgaged: all_in_set_unmortgaged = False; break
                    if all_in_set_unmortgaged: distinct_owned_color_sets.add(prop.color_set)
            
            sorted_color_sets = sorted(list(distinct_owned_color_sets), key=lambda cs_name: self._get_all_streets_in_color_set(game_state, cs_name)[0].house_cost)
            for cs_name in sorted_color_sets:
                ...
                if player.balance >= prop_to_build_on.house_cost + 800:
                    return [BuildAction(player=player, street=prop_to_build_on, quantity=1)]
        
        # --- 5. Main Turn Action: Roll Dice or Buy/Auction Property Landed On ---
        ...
        if isinstance(current_tile_obj, Property) and current_tile_obj.owner is None:
            ...
            is_priority_target = isinstance(current_tile_obj, (Utility, Railroad))
                ...
                if is_priority_target:
                    if player.balance >= current_tile_obj.purchase_cost:
                        ...
                        props_to_mortgage_for_buy = [
                            ...
                            if not p.is_mortgaged and not isinstance(p, (Utility, Railroad)) # Avoid mortgaging other Utils/RRs
                        ]
